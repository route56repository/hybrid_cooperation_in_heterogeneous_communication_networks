...
class Scenario4(Scenario):
	...
	def __init__(self):
		super().__init__()
		# Initialize network parameter parameters for each step
		i = 0
	...
	def get_best_arm(self, n):
		...
		for arm in range(self.m):
			QoE[arm] = self.getQoE(self.getTH(arm, n)*self.meanTH*10, self.getLR(arm, n)*(self.alphaLR/(self.alphaLR+self.betaLR)), self.getDL(arm, n)*self.meanDL, n)
