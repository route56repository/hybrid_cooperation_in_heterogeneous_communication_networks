...
def DiscountedCubicContextualUCB1(scenario, maxG, verbose = True):
    ...
    for i in range(NRuns):
        ...
        while n < NSteps:
            ...
            for arm in range(m):
                ...
                U = np.sqrt(max(2 * np.log(n) * np.dot(np.dot(phi[arm], B[arm]), phi[arm]), 0))
                I[arm] = Q + U
            ...
            while (j < nu) and ((j+n) < NSteps):
                ...
                tmp = tmp/(1 + (1/(discount_factor**2))*np.dot(np.dot(phi[k], B[k]), phi[k]))
                B[k] = (1/discount_factor**2)*B[k] - (1/(discount_factor**4))*tmp
        ...
        exec_time[i] = t1-t0
        if verbose:
            print("Algorithm: Discounted Cubic Contextual UCB1. Run " + str(i+1) + "/" + str(NRuns) + " completed. Final Regret: " + str(maxG[NSteps-1]-G[i, NSteps-1]) + " Average Best-Action %: " + str(sum(BA[i, :])/NSteps) + ". Execution time: " + str(exec_time[i]) + " sec.")
