...
def QRDRLSCubicContextualUCB1(scenario, maxG, verbose = True):
    ...
    for i in range(NRuns):
        ...
        exec_time[i] = t1 - t0
        if verbose:
            print("Algorithm: QRD RLS Cubic Contextual UCB1. Run " + str(i+1) + "/" + str(NRuns) + " completed. Final Regret: " + str(maxG[NSteps-1]-G[i, NSteps-1]) + " Average Best-Action %: " + str(sum(BA[i, :])/NSteps) + ". Execution time: " + str(exec_time[i]) + " sec.")
