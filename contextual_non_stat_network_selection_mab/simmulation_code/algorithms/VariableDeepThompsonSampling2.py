...
def VariableDeepThompsonSampling2(scenario, maxG, verbose=True):
				...
				while (j<nu) and ((j+n)<NSteps):
					...
					if previous_sign == sign: # decrease memory
						...
						if accumulated_m1 == m1:
							...
							nn.create_model(m)
							if n>50:
								nn.train(observed_triplets[(n-50):n], num_epochs=50)
							else:
								nn.train(observed_triplets[0:n], num_epochs=50)
							accumulated_m1 = 0
					else:
						accumulated_m0 += 1
						accumulated_m1 = 0
