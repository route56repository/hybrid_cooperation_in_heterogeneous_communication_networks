...
def RegularizedRLSLinearContextualUCB1(scenario, maxG, verbose = True):
    ...
    for i in range(NRuns):
        ...
        for arm in range(m):
            for step in range(nm):
                ...
                if to_regularize < 0:
                    B[arm] = B[arm] + to_regularize * np.identity(len(context))
        ...
        while n < NSteps:
            ...
            while (j < nu) and ((j+n) < NSteps):
                ...
                if to_regularize < 0:
                    B[k] = B[k] + to_regularize * np.identity(len(context))
        ...
        exec_time[i] = t1-t0
        if verbose:
            print("Algorithm: Regularized RLS Linear Contextual UCB1. Run " + str(i+1) + "/" + str(NRuns) + " completed. Final Regret: " + str(maxG[NSteps-1]-G[i, NSteps-1]) + " Average Best-Action %: " + str(sum(BA[i, :])/NSteps) + ". Execution time: " + str(exec_time[i]) + " sec.")
