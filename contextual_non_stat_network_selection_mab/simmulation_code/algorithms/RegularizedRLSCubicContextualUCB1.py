...
def RegularizedRLSCubicContextualUCB1(scenario, maxG, verbose = True):
    ...
    for i in range(NRuns):
        ...
        for arm in range(m):
            for step in range(nm):
                ...
                to_regularize = 0
                for pos in range((phi[arm]).size):
                    val = (B[arm])[pos][pos]
                    if val < 0 and val < to_regularize:
                        to_regularize = val 

                if to_regularize < 0:
                    B[arm] = B[arm] + to_regularize * np.identity((phi[arm]).size)
        ...
        while n < NSteps:
            ...
            while (j < nu) and ((j+n) < NSteps):
                ...
                to_regularize = 0
                for pos in range((phi[k]).size):
                    val = (B[k])[pos][pos]
                    if val < 0 and val < to_regularize:
                        to_regularize = val 

                if to_regularize < 0:
                    B[k] = B[k] + to_regularize * np.identity(phi[k].size)
        ...
        exec_time[i] = t1-t0
        if verbose:
            print("Algorithm: Regularized RLS Cubic Contextual UCB1. Run " + str(i+1) + "/" + str(NRuns) + " completed. Final Regret: " + str(maxG[NSteps-1]-G[i, NSteps-1]) + " Average Best-Action %: " + str(sum(BA[i, :])/NSteps) + ". Execution time: " + str(exec_time[i]) + " sec.")
