...
class ContextualNN3():
	...
	def train(self, observed_triplets, num_epochs=30):
		...
		X = np.zeros((array_len, self.m)) # xa_pairs
		Y = np.zeros(array_len) # observed rewards
		for i in range(array_len):
			triplet = observed_triplets[i]
			X[i, triplet[0]] = 1 # triplet[1] = arm (starts from zero). FOr categorical encoding, one input for each arm and activate the according signal
			Y[i] = triplet[1]
	...
	def predict(self, arm):
		...
		predict_array = np.zeros((1, self.m))
		predict_array[0, arm] = 1
